...
import { GenericDevice, DeviceOptions, WidgetType } from './genericDevice'
...
export class device extends GenericDevice {
    constructor(source: Source, instanceFullname: string, id: ID, attribute: string, name: string, initObject: InitObject, options?: DeviceOptions) {
        super(source, 'device', instanceFullname, id, attribute, name, initObject, options);
    }

    createInstance(configLoader: ConfigLoader, instanceFullname: string, initObject: InitObject): DomoModule {
        return new device(initObject.source, instanceFullname, initObject.id, initObject.attribute, initObject.name, initObject, {
